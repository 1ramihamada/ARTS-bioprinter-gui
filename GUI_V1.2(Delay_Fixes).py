...
PROFILE_DIR = os.path.join(os.path.dirname(__file__), "saved_profiles")
...
class BioPrinterMainFrame(wx.Frame):
    ...
    def save_registration_profile(self, name: str):
        """Dump current registration state to saved_profiles/{name}.json."""
        data = {
            "corners": self.corners,                      # list of 4 (x,y,z)
            "initial_position": self.initial_position,    # (x0,y0)
            "initial_position_z": self.initial_position_z # z0
            # you could also store transform_R if you want...
        }
    ...
    def load_registration_profile(self, name: str):
        """Load registration state from saved_profiles/{name}.json."""
        path = os.path.join(PROFILE_DIR, f"{name}.json")
        with open(path) as f:
            data = json.load(f)
        self.corners             = [tuple(c) if c else None for c in data["corners"]]
        self.initial_position    = tuple(data["initial_position"])
        self.initial_position_z  = data["initial_position_z"]
        # recompute transform if you need, or load that too...
    ...
    def _txt_profiles_dir(self):
        root = os.path.join(os.path.dirname(__file__), "converted_gcode")
    ...
    def on_load_txt_file(self, event):
        ...
        dlg = wx.FileDialog(
            self,
            message="Open .txt file with X/Y/Z/F lines",
            defaultDir=self._txt_profiles_dir(),     # <── here
    ...
    def calculate_travel_parameters(current_point, next_point, desired_feedrate):
        ...
        delay = (60.0 * distance) / desired_feedrate
#        min_delay = 0.00  # enforce at least 50 ms delay for very short moves
#        if delay < min_delay:
#            delay = min_delay
        return delay
    ...
    def run_simple_print(self):
        ...
        self.send_gcode(f"G1 X{x_coords[0]} Y{y_coords[0]} Z{z_coords[0]} F7800")
        start_delay = self.calculate_travel_parameters(
            (0, 0, 0),  # assuming homed origin
        ...
        for i in range(len(x_coords)):
            ...
            if i < len(x_coords) - 1:
                ...
                adaptive_delay = self.calculate_travel_parameters(current_point, next_point, F_coords[i+1])
            else:
    ...
    def run_interval_print(self):
        ...
        start_delay = self.calculate_travel_parameters(
            (0, 0, 0),  # or actual nozzle position if tracked
            (x_coords[0], y_coords[0], z_coords[0]),
            F_coords[0]
        )
        time.sleep(start_delay)
    ...
    def run_registration_print(self):
        ...
        self.send_gcode("G1 Z10")
        self.send_gcode("G28 X Y")
        time.sleep(0.5)

        # 3) Compute the first transformed point and its feedrate
        rx0, ry0, rz0, fv0 = self.raw_gcode[0]
        # center & rotate into mold frame
        v0   = np.array([rx0 - x_start, ry0 - y_start])
        xy0  = R @ v0 + np.array([x0, y0])
        xf0, yf0 = xy0
        zf0      = rz0 + z0

        self.send_gcode("G90")
        self.send_gcode(f"G1 Z{z0+10:.2f} F1000")
        delay1 = self.calculate_travel_parameters(
            # you’re starting *at* the drop point (x0,y0,z0)
            (0, 0, 0),
            # and ending at the first transformed raw point
            (0, 0, 10),
            # using the feedrate from the raw_gcode line
            1000
        )
        time.sleep(delay1)
        self.log(f"Delay 1: {delay1:.3f}, x0: {x0:.3f}, y0: {y0:.3f}")
        self.send_gcode(f"G1 X{xf0:.2f} Y{yf0:.2f} F7800")
        delay2 = self.calculate_travel_parameters(
            # you’re starting *at* the drop point (x0,y0,z0)
            (0, 0, 10),
            # and ending at the first transformed raw point
            (xf0, yf0, 10),
        ...
        time.sleep(delay2)
        self.log(f"Delay 2: {delay2:.3f}, xf0: {xf0:.3f}, yf0: {yf0:.3f}")
        self.send_gcode(f"G1 Z{z0:.2f} F1000")
        delay3 = self.calculate_travel_parameters(
            # you’re starting *at* the drop point (x0,y0,z0)
            (xf0,  yf0, 10),
            # and ending at the first transformed raw point
            (xf0, yf0, 0),
        ...
        time.sleep(delay3 + 0.5)
        self.log(f"Delay 3: {delay3:.3f}")
        # 5) Now it’s safe to start extruding
        self.log("Starting print loop…")

        # 6) Loop: center → rotate → translate
...
class SettingsDialog(wx.Dialog):
    ...
    def on_set_delays(self, event):
        try:
            ...
            self.mainframe.log(
                f"Delays updated: travel={trav_val}, dispense={disp_val}"
            )
...
class RegistrationDialog(wx.Dialog):
    ...
    def __init__(self, parent, title="Registration"):
        super().__init__(parent, title=title, size=(420, 360))
        ...
        btn_load = wx.Button(panel, label="Load Profile")
        btn_save = wx.Button(panel, label="Save Profile")

        btn_load.Bind(wx.EVT_BUTTON, self.on_load_profile)
        btn_save.Bind(wx.EVT_BUTTON, self.on_save_profile)

        prof_row.Add(btn_load, 1, wx.EXPAND | wx.RIGHT, 5)
        prof_row.Add(btn_save, 1, wx.EXPAND)

        sizer.Add(prof_row, 0, wx.EXPAND | wx.ALL, 5)

        # ── Clear All and Compute Transformation Row ──
        btn_row = wx.BoxSizer(wx.HORIZONTAL)

        btn_clear = wx.Button(panel, label="Clear")
        btn_clear.Bind(wx.EVT_BUTTON, self.on_clear)

        btn_compute = wx.Button(panel, label="Compute")
    ...
    def _profiles_dir(self):
        root = os.path.join(os.path.dirname(__file__), "saved_profiles")
        os.makedirs(root, exist_ok=True)
        return root

    def on_load_profile(self, evt):
        dlg = wx.FileDialog(
            self,
            message="Choose a saved-profile JSON file",
            defaultDir=self._profiles_dir(),
        ...
        try:
            ...
            self.mainframe.corners           = data["corners"]
            self.mainframe.initial_position   = tuple(data["initial_position"][:2])
            self.mainframe.initial_position_z = data["initial_position"][2]

            # refresh UI / plot
            self._load_existing()
            self.mainframe.plot_initial()
            self.mainframe.log(f'Loaded profile "{os.path.basename(path)}"')
        except Exception as e:
            wx.MessageBox(f"Could not load profile:\n{e}", "Error",
                          style=wx.ICON_ERROR)

    def on_save_profile(self, evt):
        # gather what we have
        if any(c is None for c in self.mainframe.corners):
            wx.MessageBox("Please register all four corners first.",
                          "Incomplete", style=wx.ICON_WARNING)
            return
        if self.mainframe.initial_position is None:
            wx.MessageBox("Please register the initial position.",
                          "Missing initial position", style=wx.ICON_WARNING)
            return

        data = {
            "corners":           self.mainframe.corners,
            "initial_position":  list(self.mainframe.initial_position) +
                                  [self.mainframe.initial_position_z],
        }
        ...
        try:
            with open(path, "w") as f:
                json.dump(data, f, indent=2)
            self.mainframe.log(f'Saved profile to "{os.path.basename(path)}"')
        except Exception as e:
            wx.MessageBox(f"Could not save profile:\n{e}", "Error",
                          style=wx.ICON_ERROR)
